...
function Header() {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-10 md:flex ${menuOpen ? 'h-auto' : 'h-auto '} transition-all duration-300 shadow-sm justify-between items-center px-1 py-1 md:px-5 md:py-3 w-full bg-yellow-500`}>
      ...
      <div className={` flex-col md:flex md:flex-row md:items-center justify-end md:justify-end  ${menuOpen ? 'flex' : 'hidden'} w-full mt-5 md:mt-0`}>
        {user ? (
          <div className="flex justify-end md:flex-row md:justify-end items-center w-full gap-5">
            <a href="/create-trip">
              <button className="bg-[#6a3002] border-none text-white font-bold cursor-pointer hover:bg-[#4f2b0e] w-42">Generate Trip</button>
            </a>
            <a href="/my-trips">
